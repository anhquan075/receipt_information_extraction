...
def extract_info(img_path, txt_path):
    annot_path = os.path.join(txt_path)
    ...
    a = [field_value[:-3] for field_name, field_value in list_field_values.items()]
...
if __name__ == "__main__":
    ...
    name = "mcocr_private_145120azonx"

    annot_path = os.path.join('post_processing/' + name+".txt")
    image_path = os.path.join('post_processing/' + name+".jpg")
    ...
    #     list_field_values[field_name] += field_value + '|||'
    # print(list_field_values['ADDRESS'])
    seller, address, time, total = extract_info(image_path, annot_path)
    print('seller:', seller)